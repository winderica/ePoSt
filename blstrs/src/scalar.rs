...
impl Scalar {
    pub const fn from_u256(val: [u64; 4]) -> Scalar {
        Scalar(blst_fr { l: val })
    }

    #[inline]
    pub const fn zero() -> Scalar {
        Scalar::from_u256([0, 0, 0, 0])
    }
...
impl Scalar {
    ...
    pub fn from_bytes_le(bytes: &[u8]) -> Option<Scalar> {
    ...
    /// a scalar into a `Scalar`, failing if the input is not canonical.
    pub fn from_bytes_be(bytes: &[u8]) -> Option<Scalar> {
        let mut raw = blst_scalar::default();
